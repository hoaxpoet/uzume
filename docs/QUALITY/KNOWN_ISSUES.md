...
| BUG-147 | P2 · diagnosed 2026-09-25; fix in progress (BUG147.1) | test-infra / resource-management | **The app test host sometimes crashes (exit 65) when the app tests run straight after the engine suite. Three DS.6 tests build an `NSWindow` in code and `close()` it, which releases it once more than ARC expects.** Detail below |
...
**Severity:** P2 · **Domain:** `test-infra` (UzumeAppTests) · **Failure class:** `resource-management` (an Objective-C over-release) · **Status:** Diagnosed; fix in progress (BUG147.1) · **Related:** BUG-072 (another way the app test host dies, exit 65)
