// SongMoodBetaPlaylistTests — BUG-143 verification on real music. Env-gated: it needs a
...
@Suite("BUG-143 stored mood vs the production chain (env-gated)")
struct SongMoodBetaPlaylistTests {
    private struct Entry: Decodable { let trackProfile: TrackProfile; let decodedDuration: Double? }
