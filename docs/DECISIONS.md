...
| D-245 | Accepted | Scorer: section weight gated on real section data; undeclared stem affinity scores the mean deviation; immediate nudge walks scorer-ranked order |
...
3. **The immediate nudge walks scorer-ranked order**, as `PlaybackActionRouting` already specified;
   it had sorted the catalog by name, so from the Waveform launch default "next" produced Witchlight
   (the only name after "Waveform") and then wrapped to Arachne — the sequence Matt asked about.
