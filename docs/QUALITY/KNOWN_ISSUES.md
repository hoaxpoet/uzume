...
**Severity:** P2 · **Domain:** `dsp.stem` / `orchestrator` · **Failure class:** `sample-rate` · **Status:** Fixed (BUG141.1, `3fc96385` + `f46a67f1`), unmerged · **Related:** BUG-116 (the same class in the stem series' slicing, v11), BUG-140 (the same class in the drums grid, v14 on `claude/bug140-2`)
...
**Fix.** Both sites use `separator.outputSampleRate ?? preview.sampleRate`, which falls back the same way BUG-116's slicing does. **Cache schema v13 → v14**, because the scaled values are baked into `stem_series.bin` and `metadata.json`. ⚠ `claude/bug140-2` also bumps 13 → 14. Whichever branch merges second must go to **v15**, or cache entries written by the first merged build will be read as current.
