...
**Severity:** P2 · **Domain:** `dsp.mir` → `orchestrator` · **Failure class:** `algorithm` · **Status:** Open. Diagnosed; the fix waits on Matt's product call (which song-level statistic to store) · **Found by:** KAG.3 (branch `kag-3`) · **Related:** OBS-DS4-1, BUG-144, BUG-145, DYN.7
