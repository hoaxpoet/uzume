...
- **Sample rate:** `computeBeatGrids` analyses the drums stem at `StemSeparator.modelSampleRate`. `PersistentStemCache` schema **v13 → v14**, so cached local files re-analyse instead of replaying a time-scaled drums grid.
...
5. ✅ **Manual — PASSED 2026-09-25.** Matt, session `2026-09-25T14-29-08Z`, both local copies of Superstition: *"Membrane is locked on Superstition … looks great!"* Build confirmed from the artifacts: both cache entries written fresh at schema v14; `stems.csv` `beatClarity01` = 1.00 on all 5,659 frames (the gate's verdict: steady).
