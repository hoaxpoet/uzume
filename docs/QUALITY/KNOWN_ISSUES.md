...
**Status:** **Open — root-caused, not fixed.** Found from Matt's report 2026-09-05.
...
**Verification criteria (written before the fix).**
1. Automated: `analyzeStemSeries` at 48 kHz produces zero near-zero frames on real audio, and the frame count and `hopSeconds` still map to playback seconds correctly. A rate-parameterised regression, since 44.1 kHz alone cannot see this.
2. Automated: a poisoned cache entry is not reused after the fix.
