...
| BUG-142 | P2 · **FIXED 2026-09-25 (BUG142.1)** — generation guard; pending merge | audio / concurrency | **A Now Playing poll that is in flight when `stopObserving()` runs fires a track-change event (`previous == nil`) after observation has stopped, and repopulates `currentTrack`.** Surfaced as an intermittent CI failure of `trackChange_secondTrack_hasPrevious` (3 events, expected 2). Detail below |
...
**Severity:** P2 · **Domain:** `audio` (streaming metadata) · **Failure class:** `concurrency` · **Status:** Fixed (BUG142.1), pending merge · **Related:** BUG-024 (the same stale-surface-across-a-session-boundary class, CLAUDE.md §What NOT To Do)
