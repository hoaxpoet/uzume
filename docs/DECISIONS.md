...
| D-154 | Accepted (FFO ban retired by amendment; gate tempos fixed by BUG140.2 amendment) | Beat-irregularity exclusion mechanism; pulse becomes slow 4-beat heave |
...
> **AMENDED 2026-09-25 (BUG140.2 — Matt's option A: fix the measurement, keep the 10 % rule).** The gate compared `BeatGrid.bpm`, which `computeBPM` forms as a mean of IOIs across a full octave; a drums grid that switched between eighth and quarter level averaged to a tempo describing neither, and on the local-file path the drums grid was also analysed at the file's sample rate instead of the stems' 44.1 kHz (48 kHz files ×1.088). The gate now compares the **octave-folded median** inter-beat interval of each grid's beats (`octaveFoldedMedianBPM`); the thresholds (fold > 0.10, bar confidence < 0.2) are unchanged. Measured on a 601-track stratified re-run: corpus-estimated flag rate 25.2 % → 12.0 %; duplicate recordings whose copies disagree 34/150 → 6/150; Superstition, Penny Lane regular; Pyramid Song still flagged on the production (whole-track) path. The calibration table above predates this and was computed with the old tempos — the `BeatRegularityExclusionTests` catalog rows pin BPM pairs, not beat lists, and still hold. Ratio folds (3:2, 4:3) were measured and rejected: no metrical peak in the corpus, and a 3:2 fold un-flags Mingus. Since the amendment above, Membrane (PR.26) declares `requires_regular_beat`, and BC.1 (D-257) sends the flag to every shader as `beat_clarity01`. Evidence: KNOWN_ISSUES BUG-140.
...
**Date:** 2026-09-09 · **Increment:** PR.8 · **Status:** Accepted (Matt: "do all three")

### The decision

Three changes to `DefaultPresetScorer` and the immediate preset nudge, all from what PR.8 measured with
the production scorer on cached track profiles:

1. **The section-suitability quarter is gated on real section data.** Since section detection was
   removed (D-170) every planned segment carries `section: nil`, and a nil section scored 1.0 — a
   quarter of every total was a constant. With no section, the three live sub-scores (mood 0.30,
   tempo 0.20, stem affinity 0.25) are re-normalised to sum to 1.0; the four-weight D-032 form is kept
   verbatim for the day sections return.
2. **A preset that declares no `stem_affinity` scores the track's mean stem deviation**, not a flat
   0.5. The flat value made declaring affinity a structural advantage: on energetic material the five
   declaring presets scored 0.88–1.00 on that quarter against 0.50 for the rest and owned the top of
   every ranking. Declared presets still score over their declared stems only; the D-080 zero-profile
   guard (unconverged balance → 0.5) is unchanged.
3. ~~**The immediate nudge walks scorer-ranked order**~~ — **REVERSED 2026-09-09 (PR.8.3).** Shipped,
   and it made the walk unusable: ranked order is per-track and so unpredictable to navigate, and the
   implementation recomputed it per press, which oscillated between two presets (PR.8.2). Matt:
   *"I just want the presets to be listed in alphabetical order, so that I can easily navigate to the
   preset I want."* The walk is alphabetical again. **What survives from the fix:** it walks EVERY
   loaded preset — PR.8's `filter { $0.1 > 0 }` had dropped diagnostics and over-budget presets from
   a manual override path, which is how Arachne became unreachable. `PlaybackActionRouting`'s doc
   comment claiming "scorer-ranked order" is the thing that was wrong; the alphabetical walk is the
   intended behaviour.

### Consequences

The opener stops being a per-session coin toss on the tracks measured: on Seven Nation Army Filigree
leads by 0.035, outside the ±0.02 seeded noise. `GoldenSessionTests` were regenerated under the new
formula; their sole-family repeats are the catalog symptom those tests already document. There is
still **no opener rule** — track 1 is the best fit under an empty history — and PR.8 records that as
the design, not a gap. `PlanRankingDumpTests` (env-gated, `PLAN_DUMP_META`) prints the production
ranking for any cached profile so the next "why did X open?" is answered by the code that chose it.
...
## D-248: Root Choir retires Newton and becomes Liquid Script (ROOTCHOIR.3)

**Status:** Accepted for uncertified review · 2026-09-10

Two live reviews falsified D-247's visual premise. Removing the literal ring and bounding its
rotation did not rescue the five-root Newton field: it still read as opposed forces stalling,
retained ring traces, and did not communicate psychedelia or musical causality. The mechanism is
therefore retired rather than cosmetically tuned again. `RootChoirState`, its slot-6 app wiring,
and all five tonal routes are removed.

The replacement is Liquid Script, a shader-only `direct+mv_warp` drawing preset. Three
asynchronous tapered jewel-ink strokes are written locally into persistent linear 8-bit feedback.
Three weak, spatially separated counter-rotating eddies curl the history without rotating the
frame as a whole; `bassDev` continuously changes their advection and `beatComposite` strengthens
only the new stroke. This preserves the source reference's proven write→warp→dissolve mechanism
while deliberately replacing its polygon seed, palette, and radial composition.

The authoring source is butterchurn's built-in `Martin - liquid arrows` preset, identified in the
sidecar by author, filename, and SHA-256; only a rendered motion oracle is retained in the repo.
The final 600-frame real-music replay stayed bounded and alive (mean saturation 0.532, mean luma
...
**Status:** Accepted and executed · 2026-09-11

Matt's review of the merged PR #222 build rejected the Liquid Script replacement at the concept
level: it read as Ricercar with warp added and looked nothing like the selected
`Martin - liquid arrows` animated oracle. The implementation preserved generic feedback motion
but lost the reference's defining subject—distinct pointed heads visibly drawing long fine
S-curves that braid and dissolve. Luma bounds, route firing, changing frame hashes, and a smooth
motion signal proved that the pipeline worked; none proved that the intended subject survived.

Root Choir is therefore removed from the production catalog rather than tuned again. Its shader,
sidecar, dedicated tests, reference folder, and design document are deleted, and roster counts and
...
**Addendum (Matt, 2026-09-25: "B").** The world is not only ambient: its wind and mist **breathe with
the music's slow energy** — a heavily smoothed deviation primitive (D-026), swelling over several
seconds, never at beat rate. The beat stays the fireflies' alone (FA #67). Why: a track whose beat is
irregular or unknown leaves the swarm free, so without this nothing on screen would follow the music.
Recorded in `docs/presets/FIREFLIES_DESIGN.md` §4.3 / §5; built in FF.2.
