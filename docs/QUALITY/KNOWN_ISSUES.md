...
| BUG-115 | **RESOLVED 2026-09-08 (PR.5.2, `4d5f75ff`)** — converted to `bass_att_rel` + tanh once the fill was correct | preset.routing | **Dragon Bloom's bass breathing is an absolute threshold on AGC-normalised `f.bass` — the pattern D-026 and FA #31 ban.** `mvWarpPerVertex` reads `clamp(1.0 + 0.06*(f.bass*6.0 - 1.0), 0.97, 1.07)`; the term is only neutral at `f.bass == 1/6`, so on Bowie's *Low* (median 0.236) it sits at **1.024 median / 1.070 at p90** — a 2.4–7 % outward warp every frame against the source's 0.99951 baseline, and the response is a function of the AGC's running mean rather than of the music. **Not fixed at PR.5, deliberately:** converting the route to the signed deviation primitive `bass_rel` was measured through the production path on a real *Low* capture and made the render WORSE (clipped 0.836 → 0.868, saturation 0.265 → 0.099) — the outward push is the conveyor carrying strand colour out from the centre before the warp transfer's B-fade extinguishes it, so slowing it shrinks coverage. A correct fix has to address the fill dynamics and the routing together. Evidence: `docs/diagnostics/PR5_DRAGON_BLOOM_FIX_2026-09-05.md` §3. **Update PR.5.1 (2026-09-08, `762e8862`): the fill dynamics half has moved.** The source's `warp_18..19` dither was restored (it gates the R→G→B transfer at `(ret - 0.05)·99`, so without it pixels under 0.05 never cycle hue), taking field saturation 0.634 → 0.68–0.79 into the butterchurn oracle's 0.74–0.89 band. The falsified arm recorded here was measured against the OLD fill, where the outward push was the only thing carrying colour out before the B-fade killed it; with the transfer now firing on its own, re-running the `bass_rel` conversion is worth one attempt before this row is treated as blocked. Baseline cross-check: this row's clipped 0.836 / saturation 0.265 matches PR.5.1's independently measured 0.831 / 0.262. **RESOLVED at PR.5.2.** The re-test this note invited was run and the arm now WINS: on the 20:12 session nearWhite 0.182 → 0.091, saturation 0.396 → 0.441, clipped 0.168 → 0.150. The mechanism the original falsification missed is that the 1.024–1.070 zoom was evacuating the frame faster than the strands refilled it, and the emptied corners were then driven to black by the R→G→B fade and STUCK — below the 0.05 transfer gate no push fires, so black is an absorbing state, which the comp inverts to white. Slowing the push only looked like "shrinking coverage" while the dither was missing and the transfer could not carry colour on its own. |
| BUG-122 | P2 | preset.render | **Dragon Bloom's field develops a vertical split the reference does not have — content collects in the top half, the bottom empties and inverts to white.** Measured display luma in 8 bands, top→bottom, against the butterchurn oracle on the same comp settings: the oracle is FLAT (0.89 0.85 0.84 0.87 0.78 0.79 0.85 0.88); ours at frame 900 of the 21:12 session runs **0.40 0.33 0.25 0.21 0.61 0.92 0.90 0.88** and at frame 1200 0.79 0.62 0.63 0.69 0.77 0.94 0.94 0.96. It DEVELOPS over the track — frame 300 is still nearly flat (0.84→0.69) — so it is an accumulating asymmetry in the feedback loop, not a static framing offset. This is what remains of Matt's "sizable presence of blinding white" after PR.5.1–5.3 (nearWhite 0.241 → 0.133). **Suspect the projection, not the generator** (`feedback_check_the_projection_before_the_generator`): `mvWarpPerVertex` is symmetric in y by inspection, so the candidates are the strand vertex Y-flip (`1.0 - y*2`) against the warp mesh's top-left-origin UV convention, and the `oz` perspective fold. **DIAGNOSED 2026-09-08, not yet fixed.** The instrumentation was run and the space is now partitioned. **The warp loop is NOT the cause:** run with strand alpha forced to 0 from a uniform ground, the field stays symmetric (top/bottom ratio **1.08**); with strands it is **1.83**. **The strands' geometry is not the cause either** — simulating the projection over the real session, their landing positions are balanced (52/48), their segment-length ink COVERAGE is balanced (52.7/47.3, ratio 1.11), and their injected COLOUR is uniform across all eight bands (0.88–0.91, no top/bottom difference). **The cause is the TEMPORAL distribution of the ink.** `oy = s * modK` is a one-sided ramp, so each strand is a coherent tumbling ribbon rather than a symmetric cloud, and the rotation rates are 0.2–1.5 rad/s. Per-frame on the real session the top-half ink share is **p10 0.002 / p50 0.512 / p90 0.948** — mean exactly 0.500, but almost bimodal: at any instant nearly all the ink is in ONE half, and it dwells there for seconds. The unfed half drains below the R→G→B transfer's hard `(ret-0.05)·99` gate, where no push fires and it STICKS muddy — the absorbing state PR.5.2 identified. So the mean is balanced and the field is not, which is why every aggregate measure looked fine. **Fix direction, unvalidated:** either stop the drained region sticking (raise the floor / break the absorbing state) or make ink spread faster than the dwell time. **Do NOT fix on inspection — nine hypotheses have been falsified on this defect.** |
